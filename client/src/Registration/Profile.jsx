...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://keystate-1.onrender.com/update/${currentUser._id}`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const res = await fetch(`https://keystate-1.onrender.com/delete/${currentUser._id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleSignOut = async () => {
    try {
      ...
      const res = await fetch('https://keystate-1.onrender.com/signout');
      const data = await res.json();
      if (!res.ok) {
  ...
  const handleShowListings = async () => {
    try {
      ...
      const token = localStorage.getItem('user:token');
      const res = await fetch(`https://keystate-1.onrender.com/listings/${currentUser._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`, // Include the authentication token in the request headers
        },
      });
  ...
  const handleListingDelete = async (listingId) => {
    try {
      const token = localStorage.getItem('user:token');
      const res = await fetch(`https://keystate-1.onrender.com/api/listing/deleteListing/${listingId}`, {
        method: 'DELETE', 
        headers: {
          'Authorization': `Bearer ${token}`, // Include the authentication token in the request headers
        },
