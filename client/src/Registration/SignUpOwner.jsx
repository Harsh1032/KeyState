...
const SignUpOwner = ({handleSwitchSignupType, signupType}) => {
    ...
    const registerOwner = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await fetch('https://keystate-1.onrender.com/signUpOwner', {
                method: 'POST',
                headers: {
  ...
  return (
    <>
        <form onSubmit={registerOwner} className="flex flex-col gap-4">
            <h1 className="text-3xl text-center font-semibold">Owner Signup Form</h1>
            ...
            <div className='flex justify-between mt-1 '>
                <Link to='/login'><p className=' text-black text-lg hover:underline font-medium transition ease-in-out delay-150 duration-300 cursor-pointer'> Already have an account?</p></Link>
